...
func TestConvertedCommandsRender(t *testing.T) {
	casos := []struct {
	...
	}{
		{
			...
			prms:     map[string]string{"limit": "1000", "dry_run": "false", "rpm": "60"},
			contains: "--limit 1000",
			lacks:    "--dry-run",
		},
		{
	...
	for _, c := range casos {
		t.Run(c.name, func(t *testing.T) {
			got, err := execution.Render(c.command, c.prms)
			if err != nil {
